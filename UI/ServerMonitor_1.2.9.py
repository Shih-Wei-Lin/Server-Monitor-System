...
from utils import get_status_color,inject_custom_css, create_progress_bar, create_chart,create_progress_bar_disk,create_open_new_page_button
...
def display_latest_server_connectivity(connection,latest_check_time):
    ...
    formatted_time = latest_check_time.strftime('%Y-%m-%d %H:%M:%S') if latest_check_time else "未知"
...
def show_server_data(connection, row, latest_timestamp):
    ...
    st.markdown("<div style='margin-top: 0.6em;'></div>", unsafe_allow_html=True)  # 添加0.6em的上边距
    st.markdown("""
        <style>
...
def show_statistics(connection, server_ids, latest_average_time):
    ...
    if start_date and end_date:
        ...
        # 創建和顯示圖表
        fig_CPU = go.Figure(data=cpu_traces, layout=layout("Servers CPU Usage", 'CPU(%)'))
        fig_MEM = go.Figure(data=mem_traces, layout=layout("Servers Memory Usage", 'Memory (%)'))
...
def show_calculator():
    ...
    with col1:
        ...
        with col3:
            # 單位選擇框
            frequency_unit = st.selectbox("選擇頻率單位", ['Hz', 'kHz', 'MHz', 'GHz', 'THz'], index=3, key='frequency_unit_display')
            
            if st.session_state.switch:
                frequency = st.number_input("頻率 (f)", min_value=0.0, value=1.0, step=0.1, key='frequency')
                wavelength = None
            else:
                wavelength = st.number_input("波長 (λ)", min_value=0.0, value=300.0, step=0.1, key='wavelength')
                frequency = None
        
        with col4:
            # 計算波長和頻率
            wavelength_unit = st.selectbox("選擇波長單位", ['nm', 'μm', 'mm', 'cm', 'm'], index=4, key='wavelength_unit_display')
...
def main():
    ...
    # 侧边栏全宽度无外框按钮用来导航
    st.sidebar.markdown(create_open_new_page_button("公告欄 ",bulletin_ip), unsafe_allow_html=True)
