...
class AutoExecutionerApp:
    ...
    def setup_tab1(self):
        ...
        # 執行次數
        tk.Label(frame, text="執行次數:", font=("Arial", 16)).grid(
            column=0, row=0, padx=10, pady=20, sticky=tk.W
        )
        tk.Entry(frame, width=5, textvariable=self.count, font=("Arial", 16)).grid(
            column=1, row=0, padx=5
        )

        # 執行時間
        tk.Label(frame, text="每次間隔:", font=("Arial", 16)).grid(
        ...
        tk.Entry(min_frame, width=5, textvariable=self.min, font=("Arial", 16)).pack(
            side=tk.LEFT
        )
        tk.Label(min_frame, text="分", font=("Arial", 16)).pack(side=tk.LEFT, padx=5)

        # 秒數
        tk.Entry(min_frame, width=5, textvariable=self.sec, font=("Arial", 16)).pack(
            side=tk.LEFT, padx=10
        )
        tk.Label(min_frame, text="秒", font=("Arial", 16)).pack(side=tk.LEFT, padx=5)

        # 執行檔選擇
        tk.Label(frame, text="執行檔:", font=("Arial", 16)).grid(
            column=0, row=2, padx=10, pady=20, sticky=tk.W
        )

        self.file_var = tk.StringVar(
            value=os.path.join(DefaultConfig.FILE_PATH, DefaultConfig.FILE_NAME)
        )
        file_entry = tk.Entry(
            frame, textvariable=self.file_var, width=30, font=("Arial", 12)
        )
        ...
        tk.Button(
            button_frame, text="執行", font=("Arial", 16), command=self.execute
        ).pack(side=tk.LEFT, padx=20)
        tk.Button(
            button_frame, text="清除", font=("Arial", 16), command=self.clear
        ).pack(side=tk.LEFT, padx=20)
        tk.Button(
            button_frame,
            text="保存當前設定",
    ...
    def setup_tab2(self):
        ...
        # 檢查連接設定
        tk.Label(frame, text="檢查連接間隔:", font=("Arial", 16)).grid(
        ...
        tk.Entry(
            check_frame, width=5, textvariable=self.check_min, font=("Arial", 16)
        ).pack(side=tk.LEFT)
        tk.Label(check_frame, text="分", font=("Arial", 16)).pack(side=tk.LEFT, padx=5)

        tk.Entry(
            check_frame, width=5, textvariable=self.check_sec, font=("Arial", 16)
        ).pack(side=tk.LEFT, padx=10)
        tk.Label(check_frame, text="秒", font=("Arial", 16)).pack(side=tk.LEFT, padx=5)

        # 數據獲取設定
        tk.Label(frame, text="數據獲取間隔:", font=("Arial", 16)).grid(
            column=0, row=1, padx=10, pady=10, sticky=tk.W
        )
        ...
        tk.Entry(
            extract_frame, width=5, textvariable=self.extract_min, font=("Arial", 16)
        ).pack(side=tk.LEFT)
        tk.Label(extract_frame, text="分", font=("Arial", 16)).pack(
            side=tk.LEFT, padx=5
        )

        tk.Entry(
            extract_frame, width=5, textvariable=self.extract_sec, font=("Arial", 16)
        ).pack(side=tk.LEFT, padx=10)
        tk.Label(extract_frame, text="秒", font=("Arial", 16)).pack(
            side=tk.LEFT, padx=5
        )

        # 檢查連接檔案
        tk.Label(frame, text="檢查連接檔:", font=("Arial", 16)).grid(
            column=0, row=2, padx=10, pady=10, sticky=tk.W
        )

        self.check_file_var = tk.StringVar(
            value=os.path.join(
                DefaultConfig.FILE_PATH, DefaultConfig.FILE_CHECK_CONNECT
            )
        )
        check_entry = tk.Entry(
            frame, textvariable=self.check_file_var, width=30, font=("Arial", 12)
        )
        check_entry.grid(column=1, row=2, columnspan=3, padx=5, sticky=tk.W)

        check_browse_button = tk.Button(
            frame, text="瀏覽", command=lambda: self.browse_file("check")
        )
        check_browse_button.grid(column=4, row=2, padx=5)

        # 數據獲取檔案
        tk.Label(frame, text="數據獲取檔:", font=("Arial", 16)).grid(
            column=0, row=3, padx=10, pady=10, sticky=tk.W
        )

        self.extract_file_var = tk.StringVar(
            value=os.path.join(DefaultConfig.FILE_PATH, DefaultConfig.FILE_EXTRACT)
        )
        extract_entry = tk.Entry(
            frame, textvariable=self.extract_file_var, width=30, font=("Arial", 12)
        )
        extract_entry.grid(column=1, row=3, columnspan=3, padx=5, sticky=tk.W)

        extract_browse_button = tk.Button(
            frame, text="瀏覽", command=lambda: self.browse_file("extract")
        )
        ...
        tk.Button(
            button_frame, text="清除", font=("Arial", 16), command=self.clear_tab2
        ).pack(side=tk.LEFT, padx=20)
        tk.Button(
            button_frame,
            text="保存當前設定",
    ...
    def execute(self):
        ...
        try:
            ...
            try:
                util.periodical_execution(
                    file_path, min_val, sec_val, count_val, log_callback=self.log
                )
    ...
    def toggle_continuous(self):
        ...
        if not self.continuous_running:
            try:
                ...
                self.log2("開始連續執行")
                self.log2(
                    f"檢查連接檔案: {check_file}, 間隔: {check_min_val}分{check_sec_val}秒"
                )
                self.log2(
                    f"數據獲取檔案: {extract_file}, 間隔: {extract_min_val}分{extract_sec_val}秒"
                )
    ...
    def run_continuous(
        self, check_file, extract_file, check_min, check_sec, extract_min, extract_sec
    ):
        """在新線程中運行連續執行功能"""
        try:
            ...
            self.continuous_running = False
            self.root.after(
                0, lambda: self.continuous_button.config(text="開始連續執行")
            )
