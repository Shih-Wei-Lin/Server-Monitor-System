...
async def update_database(
    ...
    async with db_pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            for ip in active_ip:
                await cur.execute(
                    "INSERT INTO active_ip (server_id, ip_address, timestamp) VALUES (%s, %s, %s)",
                    (server_id, ip, current_time),
                )
            # await conn.commit()


async def get_servers_from_db(db_pool):
    servers = []
    async with db_pool.acquire() as conn:
        async with conn.cursor() as cur:
            # 修改查询以包括连通性检查，并且只抓取最新的时间戳且是可连接的服务器
            await cur.execute(
                """
                SELECT s.host
                FROM servers s
                JOIN (
                    SELECT server_id, MAX(last_checked) as last_checked
                    FROM server_connectivity
                    WHERE is_connectable = TRUE
                    GROUP BY server_id
                ) as latest_checks ON s.server_id = latest_checks.server_id
                JOIN server_connectivity sc ON s.server_id = sc.server_id
                    AND sc.last_checked = latest_checks.last_checked
                    AND sc.is_connectable = TRUE
            """
            )
...
async def main():
    async with get_db_pool() as db_pool:
        # 获取可连接的服务器列表
        server_ips = await get_servers_from_db(db_pool)
        if not server_ips:
