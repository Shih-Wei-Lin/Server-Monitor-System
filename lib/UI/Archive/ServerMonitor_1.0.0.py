...
def show_server_data(connection, row, latest_timestamp):
    ...
    with st.expander(f" {server_id} more info", expanded=False):
        # Add a radio button for user to choose data to display
        choice = st.radio(
            " ",
            ("Account", "User"),
            key=f"choice_{server_id}",  # 確保每個 radio 組的 key 唯一
        ...
        try:
            ...
            if choice == "Account":
                if active_users:
                    ...
                    df_active_users = df_active_users.rename(
                        columns={"username": "Account"}
                    )
                    df_active_users = df_active_users.drop(columns=["timestamp"])
                    st.table(df_active_users)
                else:
                    st.info(f"No active account at server {server_id}")
            elif choice == "User":
                if active_usernames:
                    df_active_usernames = pd.DataFrame(active_usernames)
                    df_active_usernames = df_active_usernames.rename(
                        columns={"user_name": "User"}
                    )
                    df_active_usernames = df_active_usernames.drop(
                        columns=["timestamp"]
                    )
                    st.table(df_active_usernames)
                else:
                    st.info(f"No active user at server {server_id}")
...
def show_server_monitor_system():
    ...
    with tab3:
        ...
        with col1:
            ...
            # 用户选择服务器ID
            selected_server_id = st.selectbox("Choose Server", server_ids)
        ...
        if start_date and end_date and selected_server_id is not None:
            data = get_server_metrics_averages(
                connection, selected_server_id, start_date, end_date
            )
            if data:
                df = pd.DataFrame(data)
                df.rename(columns={"average_timestamp": "Time"}, inplace=True)

                # 使用 Streamlit expander 顯示原始數據
                with st.expander(" Raw info", expanded=False):
                    st.write(df)
                ...
                # 設置圖表的佈局
                layout = go.Layout(
                    title=f"Server {selected_server_id} Average Usage：",
                ...
                # 在 Streamlit 中顯示圖表，並使用容器的全寬
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.write("NO DATA in Selected Time Range。")
