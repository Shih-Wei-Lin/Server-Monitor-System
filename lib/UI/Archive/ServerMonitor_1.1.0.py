...
def version_updates():
    # 假設有一個更新日誌的資料庫表格或其他資料來源
    # 以下是一個示例數據，您需要從資料庫或其他來源取得實際數據
    updates_data = [
        {
            "version": "1.1.0",
            "release_date": "2023-08-26",
            "features": "Change Account and User display interface & Add version_update page & Change SQL server and Website at 111,refresh code at 112 for stable ",
        },
        {
            "version": "1.0.0",
    ...
    for update in updates_data:
        ...
        feature_list = update["features"].split("&")
        for i, feature in enumerate(feature_list, start=1):
            st.write(f"{i}. {feature.strip()}")  # strip() 移除頭尾空白
